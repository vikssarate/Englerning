...
<title>English Lerning App</title>
<style>
...
/* ===== STORAGE & CONFIG ===== */
const LSK="mem_spaced_v11_altC"; // NEW localStorage key
...
function renderCycleFooterHTML(ch){
  ...
  const total = 3 + 3 + SCHEDULE_GROUPS.reduce((a,g)=>a+g.reps,0);
  ...
  return `
    <div class="prog">
      ...
      <div class="labels">
        ...
        <span>Spaced ${spaced}/${total-6}</span>
        <span>Weekly repeats forever</span>
      </div>
    </div>
  `;
}
...
function renderDueBar(){
  ...
  dueBar.onclick = (e)=>{
    ...
    if(act==="review"){
      ...
      save(); // re-renders strip + tables
    }else{
      // just jump to the chunk
...
function openTopic(subId,topicId,highlightChunkId=null){
  ...
  // Jump to and flash the chosen chunk, if provided
  if(highlightChunkId){
...
$("#exportJson").addEventListener("click",()=>{
  ...
  a.href=url; a.download=`mem_spaced_altC_${todayYMD()}.json`; // NEW filename prefix
